...
<div class="gridContainer clearfix">
  ...
  if(isset($_SESSION["id"])){
	  if(isset($_REQUEST["submit"])){
		  $id=$_SESSION["id"];
		  $npw=$_REQUEST["newpassword"];
		  $npw2=$_REQUEST["newcheckpassword"];
		  $nname=$_REQUEST["newname"];
		  $nemail=$_REQUEST["newemail"];
		  $flag=1;
		  if($npw=="") {echo "<script>alert('請輸入新密碼');</script>"; $flag=0;}
		  else if($npw2=="" || $npw2!=$npw) {echo "<script>alert('請再次輸入確認密碼');</script>"; $flag=0;}
		  else if($nname=="") {echo "<script>alert('請輸入會員名稱');</script>"; $flag=0;}
		  else if($nemail=="") {echo "<script>alert('請輸入新信箱');</script>"; $flag=0;}
		  if($flag==1){
			  $sql = "update members 
			  set password='$npw', name='$nname', email='$nemail' 
			  where id='$id'";
			  if(mysqli_query($link,$sql)){
				  echo "<script>alert('修改成功!按下確定自動跳轉回首頁。');</script>";
				  echo '<meta http-equiv="REFRESH" CONTENT=0;url=index.php>';
				  }
				  else{echo "<script>alert('修改失敗!');</script>";}
			  }
		}
	 if(isset($_REQUEST["cancel"])){
		...
		/*echo "<script>alert('您無權觀看此頁面');</script>";*/
		//header("Location:index.php");
		echo '<meta http-equiv="REFRESH" CONTENT=0;url=index.php>';
		}
...
<form action="update.php" method="post">
  	會員帳號： <?php echo $_SESSION["id"]; ?>
  	<br />
    新密碼：
  <input name="newpassword" type="password" class="input_text" />
    <br />
    確認密碼：
  	<input name="newcheckpassword" type="password" class="input_text" />
    <br />
    會員名稱：<input name="newname" type="text" class="input_text" />
    <br />
    性別： <?php 
	if($_SESSION["ex"]==0) echo "男";
	else echo "女"; ?>
    <br />
    生日： <?php echo $_SESSION["birthday"]; ?>
    <br />
    信箱：<input name="newemail" type="text" class="input_text" />
    <br />
    <br />
  <input name="submit" type="submit" value="確認修改" />
  <input name="cancel" type="submit" value="取消修改"/>
</form>
      </div>
